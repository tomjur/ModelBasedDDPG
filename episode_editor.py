...
class EpisodeEditor:
    def __init__(self, alter_episode_mode, pre_trained_reward, joints_dimension=4, pose_dimension=2,
                 status_dimension=3):
    ...
    def _append_to_buffers(self, current_joints, goal_joints, actions, goal_poses, status):
    ...
    def process_episodes(self, episodes, sess):
        ...
        for episode_agent_trajectory in episodes:
            ...
            # add data to buffers
            status, states, actions, rewards, goal_pose, goal_joints, workspace_image = episode_agent_trajectory
            ...
            self._append_to_buffers(
                current_joints, [goal_joints] * len(actions), actions, [goal_pose] * len(actions), one_hot_status)
        ...
        for episode_start_index, episode_agent_trajectory in zip(episode_start_indices, episodes):
            ...
            if self.alter_episode_mode == 2:
                altered_result = status, states, actions, relevant_rewards, goal_pose, goal_joints, workspace_image
